...
try {
  receiveMessageQueue = new Queue(requestNamespace, Config[messagingType].users[0].redisUrl);
  logger.debug(`SUCCESS: connected to bull queue "${requestNamespace}" at ${Config[messagingType].users[0].redisUrl}`);
} catch (error) {
  logger.error(`ERROR: could not connect to bull queue "${requestNamespace}" at ${Config.messagingType.users[0].redisUrl}`);
}
